...
create index if not exists incidents_site_status_created_idx
  on public.incidents (site_id, status, created_at desc);

create index if not exists transcript_turns_incident_order_idx
  on public.transcript_turns (incident_id, turn_order);

create index if not exists incident_actions_incident_created_idx
...
create policy "authenticated can read transcript turns"
on public.transcript_turns for select
to authenticated
using (true);

create policy "authenticated can read incident actions"
on public.incident_actions for select
to authenticated
using (true);

create policy "authenticated can read incident reports"
